...
const deleteWorld = async (filter) =>{
  try {
    let response  = await dbService.deleteMany(World,filter);
    return response;
  } catch (error){
...
const deletePiece = async (filter) =>{
  try {
    let response  = await dbService.deleteMany(Piece,filter);
    return response;
  } catch (error){
    throw new Error(error.message);
  }
...
const deleteChessMap = async (filter) =>{
  try {
    let chessmap = await dbService.findMany(ChessMap,filter);
    if (chessmap && chessmap.length){
      chessmap = chessmap.map((obj) => obj.id);

      const WorldFilter = { $or: [{ Chunks : { $in : chessmap } }] };
      const WorldCnt = await dbService.deleteMany(World,WorldFilter);

      const pieceFilter = { $or: [{ Table : { $in : chessmap } }] };
      const pieceCnt = await dbService.deleteMany(Piece,pieceFilter);

      let deleted  = await dbService.deleteMany(ChessMap,filter);
      let response = {
        World :WorldCnt,
        piece :pieceCnt,
      };
...
const deleteUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const WorldFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const WorldCnt = await dbService.deleteMany(World,WorldFilter);
      ...
      const pieceFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } },{ Owner : { $in : user } }] };
      const pieceCnt = await dbService.deleteMany(Piece,pieceFilter);

      const ChessMapFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const ChessMapCnt = await dbService.deleteMany(ChessMap,ChessMapFilter);
...
const countWorld = async (filter) =>{
  try {
    const WorldCnt =  await dbService.count(World,filter);
    return { World : WorldCnt };
  } catch (error){
    throw new Error(error.message);
  }
};
...
const countPiece = async (filter) =>{
  try {
    const pieceCnt =  await dbService.count(Piece,filter);
    return { piece : pieceCnt };
  } catch (error){
    throw new Error(error.message);
  }
};

const countChessMap = async (filter) =>{
  try {
    let chessmap = await dbService.findMany(ChessMap,filter);
    if (chessmap && chessmap.length){
      chessmap = chessmap.map((obj) => obj.id);
      ...
      const WorldFilter = { $or: [{ Chunks : { $in : chessmap } }] };
      const WorldCnt =  await dbService.count(World,WorldFilter);
...
const countUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const ChessMapFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const ChessMapCnt =  await dbService.count(ChessMap,ChessMapFilter);
...
const softDeleteWorld = async (filter,updateBody) =>{  
  try {
    const WorldCnt =  await dbService.updateMany(World,filter);
    return { World : WorldCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeletePiece = async (filter,updateBody) =>{  
  try {
    const pieceCnt =  await dbService.updateMany(Piece,filter);
    return { piece : pieceCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeleteChessMap = async (filter,updateBody) =>{  
  try {
    let chessmap = await dbService.findMany(ChessMap,filter, { id:1 });
    if (chessmap.length){
      chessmap = chessmap.map((obj) => obj.id);

      const WorldFilter = { '$or': [{ Chunks : { '$in' : chessmap } }] };
      const WorldCnt = await dbService.updateMany(World,WorldFilter,updateBody);

      const pieceFilter = { '$or': [{ Table : { '$in' : chessmap } }] };
      const pieceCnt = await dbService.updateMany(Piece,pieceFilter,updateBody);
      let updated = await dbService.updateMany(ChessMap,filter,updateBody);
...
const softDeleteUser = async (filter,updateBody) =>{  
  try {
    ...
    if (user.length){
      ...
      const ChessMapFilter = { '$or': [{ addedBy : { '$in' : user } },{ updatedBy : { '$in' : user } }] };
      const ChessMapCnt = await dbService.updateMany(ChessMap,ChessMapFilter,updateBody);
